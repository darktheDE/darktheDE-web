...
type Category = "AWS" | "Google" | "Industry" | "Award" | "Academic";
...
export function CertificationsSection() {
  ...
  useFocusTrap(dialogRef, selectedIdx !== null, () => setSelectedIdx(null));
  ...
  const selectedCert =
    selectedIdx !== null ? visibleCertifications[selectedIdx] : null;
  const closeLightbox = () => setSelectedIdx(null);
  const goPrev = () =>
    setSelectedIdx((prev) =>
      prev !== null && prev > 0 ? prev - 1 : visibleCertifications.length - 1
    );
  const goNext = () =>
    setSelectedIdx((prev) =>
      prev !== null && prev < visibleCertifications.length - 1 ? prev + 1 : 0
    );
  ...
  return (
    ...
    >
      ...
      >
        <div className="mb-8 flex flex-col justify-between gap-5 border-y border-rule py-6 sm:flex-row sm:items-end">
          <div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {visibleCertifications.map((cert, idx) => (
            <Motion.button
              ...
              className="group overflow-hidden border border-rule bg-card text-left backdrop-blur-md transition-all duration-500 hover:border-accent/25"
            >
              ...
              <div className="p-4">
                <span className="border border-accent/20 bg-accent/10 px-2 py-0.5 font-mono text-[10px] uppercase tracking-wide text-accent">
                  {cert.category}
                </span>
                <h3 className="mt-3 line-clamp-2 text-sm font-semibold leading-snug text-text">
                  {cert.title}
                </h3>
              </div>
            </Motion.button>
        ...
        {selectedCert && (
          ...
          >
            ...
            >
              ...
              <button
                onClick={closeLightbox}
                className="absolute -top-12 right-0 bg-panel/50 p-2 text-mute transition-colors hover:text-text sm:-right-4 sm:bg-transparent"
                aria-label="Close"
              >
              ...
              <button
                ...
                className="absolute left-0 top-1/2 -translate-x-4 -translate-y-1/2 bg-black/50 p-2 text-mute backdrop-blur-sm transition-colors hover:text-text sm:-translate-x-14 sm:bg-transparent"
                aria-label="Previous"
              >
              ...
              <button
                ...
                className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 bg-black/50 p-2 text-mute backdrop-blur-sm transition-colors hover:text-text sm:translate-x-14 sm:bg-transparent"
                aria-label="Next"
              >
